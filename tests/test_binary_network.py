...
class NetworkTestCase(unittest.TestCase):
    ...
    def test_joint_distribution(self):
        ...
        sinit, a_states, a_s = bnet.simulate_eve_sparse(W, b, tau, sinit, steps * tau / N, [0, Nrec], [N], [hlp.Fsigma], beta=beta)
    ...
    def test_marginal_distribution(self):
        ...
        a_s = bnet.simulate(W, b, sinit, steps, Nrec,
                            [N], [hlp.Fsigma], beta=beta)[1]
        marginals = hlp.get_marginals(a_s, steps_warmup)
        nptest.assert_array_almost_equal(
            expected_marginals, marginals, decimal=2)

        sinit, a_times, a_s = bnet.simulate_eve_sparse(W, b, tau, sinit, steps * tau / N, [0, Nrec],
    ...
    def test_input(self):
        ...
        for i, sim in enumerate([bnet.simulate, bnet.simulate_eve_sparse]):
            if i == 0:
                ...
                sinit, a_times, a_s, a_times_ui, a_ui = sim(
                    W, b, tau, sinit.copy(), time, [0, Nrec], [N, N + Nnoise], [hlp.Ftheta, hlp.Fsigma], beta, rNrec_u=[0, Nrec_ui])
