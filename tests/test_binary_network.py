...
class NetworkTestCase(unittest.TestCase):
    ...
    def test_multiple_activation_functions(self):
        ...
        for i, sim in enumerate([bnet.simulate, bnet.simulate_eve]):
            if i == 0:
                a_s = sim(W, b, sinit, steps, Nrec, [N1, N], [F1, F2])[1]
            else:
                a_s = sim(W, b, tau, sinit, steps * tau / N,
                          Nrec, [N1, N], [F1, F2])[1]
    ...
    def test_input(self):
        ...
        b_brn[N:] = -1. * \
            ...
            1. * w / 2
        a_times_brn, a_s_brn, a_times_ui_brn, a_ui_brn = bnet.simulate_eve(
            W_brn, b_brn, tau, sinit.copy(), time, Nrec, [N + Nnoise], [hlp.theta], Nrec_ui=10)
        ...
        a_times, a_s, a_times_ui, a_ui = bnet.simulate_eve(
            W, b, tau, sinit.copy(), time, Nrec, [N, N + Nnoise], [hlp.theta, hlp.Fsigma], Nrec_ui=10)
        self.assertTrue(abs(np.mean(a_ui) + w / 2. - expected_mu_input)
                        < 0.04 * abs(expected_mu_input))
        self.assertTrue(abs(np.mean(np.std(a_ui, axis=0)) - expected_std_input)
...
class MeanfieldTestCase(unittest.TestCase):
    ...
    def test_comp_network_meanfield(self):
        ...
        W[:N, N:] = hlp.create_noise_connectivity_matrix(
            N, Nnoise, gamma, g, w, epsilon)
        W[N:, N:] = hlp.create_connectivity_matrix(
            Nnoise, w, g, epsilon, gamma)
        b = np.zeros(N + Nnoise)
        b[:N] = -w / 2.
        b[N:] = -1. * \
            hlp.get_mu_input(epsilon, Nnoise, gamma, g, w, mu_target) - w / 2.
        sinit = np.array(np.random.randint(0, 2, N + Nnoise), dtype=np.int)

        times, a_s, a_times_ui, a_ui = bnet.simulate_eve(
            W, b, tau, sinit, T, N + Nrec, [N + Nnoise], [hlp.theta], Nrec_ui=N)
        a_ui = a_ui[200:]
        a_s = a_s[200:]
