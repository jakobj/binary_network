...
class HelperRegressionTestCase(unittest.TestCase):
    ...
    def test_missing_states(self):
        ...
        a_s = np.vstack(
            [a_s, np.zeros(M * N * steps / 2).reshape(steps / 2, M * N)])
        ...
        for i in range(M):
            nptest.assert_array_almost_equal(
                expected_marginals[i], marginals[i])
    ...
    def test_normalized_positive_density_DKL(self):
        ...
        expected_dkl = [np.sum([p[j, i] * np.log(p[j, i] / q[j, i])
                                for i in xrange(4)]) for j in xrange(2)]
    ...
    def test_number_of_recorded_units(self):
        ...
        W = bhlp.create_BM_weight_matrix(
            N, np.random.beta, mean_weight, a=2., b=2.)
        b = bhlp.create_BM_biases_threshold_condition(
            N, mean_weight, mean_activity)
